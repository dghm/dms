...
- DOMESTIC (國內運輸) 4 個節點
- IMPORT/EXPORT/CROSS (進出口運輸/跨國貿易) 7 個節點 + 2 個事件

##### 1.2 節點狀態系統

- Excuted (已執行): 藍色實心圓 + 白色實心圓 + 勾選圖標
...
本系統採用「前端展示 + Serverless API + Airtable」三層架構：

1. **前端展示層**：負責接收使用者輸入（Order No. / Tracking No.）並呈現時間軸狀態。
2. **API 處理層**：由 Netlify Function 接收查詢請求、驗證參數並轉換資料格式。
3. **資料存取層**：透過 Airtable API 讀取追蹤資料，回傳前端所需之節點資訊與狀態欄位。

此架構可同時兼顧部署效率、維護彈性與後續擴充性（新增節點或狀態類型）。
...
- **核心識別欄位**：Order No.、Tracking No.
- **狀態欄位**：Current Status、Completed At、Updated At
- **節點欄位**：各時間軸節點之名稱、排序、時間戳記、啟用狀態
- **顯示設定欄位**：節點顏色、圖示、說明文字、前端顯示順序

資料欄位命名採一致格式，便於 API 映射與前端渲染，降低維護成本。
