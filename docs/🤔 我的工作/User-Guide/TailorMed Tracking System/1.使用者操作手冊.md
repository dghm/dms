...
本系統為 TailorMed 提供之專屬查詢平台，協助客戶即時追蹤貨件運輸與狀態資訊。使用者僅需輸入「Job No.」與「Tracking No.」，即可查詢對應的運輸狀態與溫度紀錄。

本手冊將說明操作方式與查詢結果的各項定義，並提供資料填寫指引與常見問題解答。
...
- 功能目的：提供客戶與合作夥伴、服務對象即時查詢冷鏈貨件的狀態與相關資訊。
- 資料來源：由【TailorMed】OMS 資料庫系統(Tracking 表單)同步更新。
