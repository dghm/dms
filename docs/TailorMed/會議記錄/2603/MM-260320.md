...
- Q：「是否在 Airtable 裡有記錄每一筆資料的歷史編輯？」by Cody
  - A: 要檢視該筆資料的歷史資料，操作如下示意
    ![alt text](01.png)
    ![alt text](02.png)
    ![alt text](03.png)
- 未來考慮要加入**預先付款（Prepayment）**功能 by Chris
- Interface 中的 Shipper/Consignee 新增的 Collection / Delivery Co. 的 **ATTNs 欄位需設為必填**
- 在不同廠商配合的專案型 Quotation 中， **EXP. Date**，因會有談季期或是年期，故需支援手動修改（Customize）by Chris
- 列印文件時 **USD 金額字體需加大以利辨示** by Chris
...
- 庫存系統流程規劃（採購 → 入庫 → 出貨）
  - 因工作邏輯尚在釐清中，後續將會再約同相關人員進行討論。

- 諾為泰(Novotech)稽核後，有提及可以開放 API 接口，讓崴宇透過資料登錄及上傳。
- 此為 B2B 的整合方案，目的在於透過 TailorMed 溫度計數據對接 Novotech 系統，考量到技術的面向以及選擇不同方法可能帶來的工作複雜度。先提供初步可行方案的文件(下方連結所示)給予參考，待後續需要進一步執行時，可再約相關人員進行討論。
  - [API Intergration](TailorMed_Novotech_API整合方案.md)

- 展示 Interface 操作文件範本第一版 [CRM > Partner Interface User Guide](<TailorMed [US] CRM Interface Guide.pdf>)
